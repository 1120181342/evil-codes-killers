...
class GaussViewSet(viewsets.ModelViewSet):
    """List and Retrieve Note instances"""
    ...
    while True:
        ...
        s = client.recv(buffSize).decode("utf-8")
        l = int(s)
        buffSize  = buffSize * (l //10 + 1)
        ...
        for i in range(1, lenth+1):
            ...
            print(tmp)
            print("----")
            record = dict(id = tmp[0], sha = tmp[1], Class = tmp[2], Time = tmp[3])
            print(record)
            gauss_list.append(record)
        print(gauss_list)
        select = "-1"
        client.send(bytes(select, "utf-8"))
        time.sleep(3)
